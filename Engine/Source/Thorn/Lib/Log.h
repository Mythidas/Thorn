...
namespace Thorn {
	...
	{
		std::println("{}[{}]: {}{}", c_GRAY, GetTime(), std::vformat(fmt, std::make_format_args(std::forward<Types>(args)...)), c_RESET);
	}

	template<typename ...Types>
	inline void Log::Info(std::string_view fmt, Types&&... args)
	{
		std::println("{}[{}]: {}{}", c_GREEN, GetTime(), std::vformat(fmt, std::make_format_args(std::forward<Types>(args)...)), c_RESET);
	}

	template<typename ...Types>
	inline void Log::Warn(std::string_view fmt, Types&&... args)
	{
		std::println("{}[{}]: {}{}", c_AMBER, GetTime(), std::vformat(fmt, std::make_format_args(std::forward<Types>(args)...)), c_RESET);
	}

	template<typename ...Types>
	inline void Log::Error(std::string_view fmt, Types&&... args)
	{
		std::println("{}[{}]: {}{}", c_RED, GetTime(), std::vformat(fmt, std::make_format_args(std::forward<Types>(args)...)), c_RESET);
	}

	inline std::string Log::GetTime()
	{
